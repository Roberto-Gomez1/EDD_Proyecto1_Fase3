/**
 * Codificar en Base64 pdf o imagen
 */
const inputElement1 = document.getElementById("input2");
inputElement1.addEventListener("change", onChange, false);
let nombreArchivo = ""
let base64String = ""
...
const filename1=[]
const permisocarnet1=[]
const permiso1=[]
function asignarPermisos(){
    let filename=[]
    let permisocarnet =[]
    let permiso=[]
    let nombre = localStorage.getItem("nombrealumno")
    let cadena = document.getElementById("permiso").value
    let arreglo = cadena.split('-')
    filename.push(arreglo[0])
    permisocarnet.push(arreglo[1])
    permiso.push(arreglo[2])
    filename1.push(filename)
    permisocarnet1.push(permisocarnet)
    permiso1.push(permiso)
    localStorage.setItem(nombre+"permiso",permiso1)
    localStorage.setItem(nombre+"permisocarnet",permisocarnet1)
    localStorage.setItem(nombre+"filename",filename1)
    localStorage.setItem(nombre+"propietario",nombre)
